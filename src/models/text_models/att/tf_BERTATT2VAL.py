...
class tf_BERTATT2VAL(ATT2VAL):
    ...
    def get_sub_model(self):
        ...
        tokenized_inputs = [tokenize(text_in)]
        # Pack input sequences for the Transformer encoder.
        seq_length = pad_len  # Your choice here.
        ...
        if mv == "0": # Se añade la estandarización a la salida y se simplifica el modelo
            ...
            bert_emb = outputs["sequence_output"]
            ht_emb = tf.keras.layers.Lambda(lambda x: x, name="word_emb")(bert_emb)

            mask_query = tf.cast(encoder_inputs["input_mask"], tf.float16) # Se obtiene la máscara del texto para un solo item
    ...
    def __create_tfdata__(self, dataframe):
        ...
        # Pack input sequences for the Transformer encoder.
        seq_length = self.DATASET.DATA["MAX_LEN_PADDING"]  # Your choice here.
        bert_pack_inputs = hub.KerasLayer(preprocessor.bert_pack_inputs, arguments=dict(seq_length=seq_length))  # Optional argument.
        encoder_inputs = bert_pack_inputs(tokenized_inputs)
        

        # Obtener ids restaurantes y generar tfdata
        rst_data = dataframe.id_item.values
        data_x1 = tf.data.Dataset.from_tensor_slices(text_data)
