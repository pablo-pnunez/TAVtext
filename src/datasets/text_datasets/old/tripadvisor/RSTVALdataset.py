...
class RSTVALdataset(TripAdvisorDataset):

    def __init__(self, config, load=None):
        TripAdvisorDataset.__init__(self, config=config, load=load)

    def get_data(self, load=["TEXT_SEQUENCES", "BOW_SEQUENCES", "TRAIN_DEV", "TEST", "WORD_INDEX", "VOCAB_SIZE", "MAX_LEN_PADDING", "TEXT_TOKENIZER", "VECTORIZER", "FEATURES_NAME", "N_RST", "STEMMING_DICT"]):
        ...
        # Si no existe, crear
            ...
            all_data = all_data.astype({'dev': 'int8', 'test': 'int8', 'rating': 'int8', 'bow': 'int32', 'seq': 'int32', 'id_restaurant': 'int32', })
