...
from src.datasets.text_datasets.tripadvisor.TripAdvisorDataset import *
...
class ATT_dataset(TripAdvisorDataset):

    def __init__(self, config, load=None):
        TripAdvisorDataset.__init__(self, config=config, load=load)

    def get_data(self, load=["TEXT_SEQUENCES", "TRAIN_DEV", "TEST", "WORD_INDEX", "VOCAB_SIZE", "MAX_LEN_PADDING", "TEXT_TOKENIZER", "N_ITEMS", "STEMMING_DICT"]):
        ...
        # Si no existe, crear
            ...
            rst_newid = pd.DataFrame(zip(r_mth_x, range(len(r_mth_x))), columns=["restaurantId", "id_item"])
            ...
            model_data = all_data.drop(columns=["date", "language", "text", "title", "url", "city", "text_base", "title_base"])
